        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="bg-slate-900 border border-slate-700/50 w-full max-w-2xl rounded-[2.5rem] p-6 md:p-10 shadow-2xl relative z-[160] animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              {/* Language Switcher */}
              <div className="flex items-center gap-1 bg-slate-950/40 p-1 rounded-xl border border-slate-800/50 backdrop-blur-md">
                {(Object.values(Language) as Language[]).map(lang => (
                  <button 
                    key={lang} 
                    onClick={() => setSettings(s => ({...s, language: lang}))}
                    className={`px-2 py-1 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-widest transition-all ${settings.language === lang ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20' : 'bg-slate-800/50 text-slate-400 hover:bg-slate-700'}`}
                  >
                    {lang === Language.EN ? 'EN' : lang === Language.BN ? 'বাংলা' : 'PT'}
                  </button>
                ))}
